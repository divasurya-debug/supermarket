...
@section('content')
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
    ...
    <a href="{{ route('admin.brands.create') }}" 
       class="bg-purple-700 text-white font-bold py-2 px-4 rounded hover:bg-purple-800 w-full sm:w-auto text-center">
      + Tambah Merek
    </a>
  </div>
            ...
            <tr class="hover:bg-gray-50">
              <td class="px-4 py-4 whitespace-nowrap">
                @if($brand->gambar)
                  <img src="{{ $brand->gambar }}" 
                       alt="{{ $brand->nama_merek }}" 
                       class="h-12 w-auto rounded shadow bg-gray-100 p-1">
                @else
                  <span class="text-gray-400 italic">Tidak ada logo</span>
                @endif
              </td>
