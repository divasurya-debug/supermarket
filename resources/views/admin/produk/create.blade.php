    ...
    <form action="{{ route('admin.produk.update', $produk->id_produk) }}" method="POST" enctype="multipart/form-data">
          ...
          <select id="id_brands" name="id_brands"
            ...
            @foreach ($brands as $brand)
              <option value="{{ $brand->id_brands }}" 
                {{ old('id_brands', $produk->id_brands) == $brand->id_brands ? 'selected' : '' }}>
                {{ $brand->nama_merek }}
              </option>
            @endforeach
          </select>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
        ...
        <div>
          ...
          <input type="number" id="jumlah_terjual" name="jumlah_terjual"
            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600"
            value="{{ old('jumlah_terjual', $produk->jumlah_terjual ?? 0) }}" min="0">
      ...
      <div class="mb-4">
        ...
        <textarea id="deskripsi" name="deskripsi" rows="4"
          class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
        @error('deskripsi')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-6">
        ...
        @if ($produk->gambar)
          <div class="mt-2">
            ...
            <img src="{{ asset($produk->gambar) }}" alt="Gambar Produk" class="h-32 rounded-md shadow">
          </div>
        @endif
