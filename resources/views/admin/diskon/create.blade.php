...
@section('content')
  <div class="bg-white p-8 rounded-lg shadow-md">
    <h2 class="text-2xl font-bold mb-6">Tambah Diskon Baru</h2>
    <form action="{{ route('admin.diskon.store') }}" method="POST">
      ...
      <div class="mb-4">
        ...
        <select id="id_produk" name="id_produk" class="w-full px-4 py-2 border rounded-lg" required>
            ...
            @foreach($products as $product)
                <option value="{{ $product->id_produk }}" {{ old('id_produk') == $product->id_produk ? 'selected' : '' }}>
                    {{ $product->nama_produk }}
                </option>
            @endforeach
        </select>
      ...
      <div class="mb-4">
        ...
        <input type="number" id="persentase_diskon" name="persentase_diskon" class="w-full px-4 py-2 border rounded-lg" value="{{ old('persentase_diskon') }}" placeholder="Contoh: 20" required>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
        <div>
            ...
            <input type="date" id="tanggal_mulai" name="tanggal_mulai" class="w-full px-4 py-2 border rounded-lg" value="{{ old('tanggal_mulai') }}" required>
        ...
        <div>
            ...
            <input type="date" id="tanggal_akhir" name="tanggal_akhir" class="w-full px-4 py-2 border rounded-lg" value="{{ old('tanggal_akhir') }}" required>
