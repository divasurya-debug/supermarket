...
@section('content')
  <div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.diskon.create') }}" class="bg-purple-700 text-white font-bold py-2 px-4 rounded hover:bg-purple-800">
      + Tambah Diskon
    </a>
  </div>
      ...
      <tbody class="divide-y divide-gray-200">
        @forelse($discounts as $discount)
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-4 whitespace-nowrap text-gray-500 font-bold text-red-500">{{ $discount->persentase_diskon }}%</td>
            <td class="px-4 py-4 whitespace-nowrap text-gray-500">{{ $discount->tanggal_mulai->format('d M Y') }}</td>
            <td class="px-4 py-4 whitespace-nowrap text-gray-500">{{ $discount->tanggal_akhir->format('d M Y') }}</td>
            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
              <a href="{{ route('admin.diskon.edit', $discount->id_diskon) }}" class="text-indigo-600 hover:text-indigo-900 mr-4">Edit</a>
              <form action="{{ route('admin.diskon.destroy', $discount->id_diskon) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin?');">
                @csrf
                @method('DELETE')
